...
ICON_PATH = "icon.png"  # page icon file in repo (optional)
...
if _side_b64:
    ...
    """
# floating preview styles (safe defaults)
custom_css += """
...
def sanitize_for_preview(svg_bytes: bytes) -> str:
    """
    Make SVG parseable and resilient for preview/generation.
    This performs conservative cleanups only: removes DOCTYPE/ENTITY,
    normalizes prefixed names to safe underscores, ensures xmlns/version/viewBox.
    """
    if not svg_bytes:
    ...
    # Replace prefixed tags/attrs with underscore to avoid invalid attr names (e.g. i:extraneous)
    ...
    # remove xmlns:prefix declarations (keep default xmlns)
    raw = re.sub(r'\s+xmlns:[a-zA-Z0-9_]+="[^"]+"', '', raw)
    # drop unknown named entities (keep core XML entities)
    ...
    # ensure we have actual <svg> element
    if not (isinstance(root.tag, str) and root.tag.lower().endswith("svg")):
        ...
        if cand is not None:
            ...
            # try to find any element whose tag endswith 'svg'
            found = None
            for el in root.iter():
    ...
    # normalize inline style attribute (dedupe entries)
    for el in root.iter():
        style = el.get("style")
        if style:
...
def render_svg_to_png(svg_text: str, scale: float = 1.0) -> bytes:
    ...
    # apply internal upscale to keep text crisp on zoom (we downscale in browser)
...
def apply_mapping_to_svg(svg_text: str, mapping: dict, record: dict) -> str:
    ...
    for text_elem in text_nodes:
        ...
        # transform support (dx,dy,scale)
        try:
            dx = float(cfg0.get("dx", 0))
            dy = float(cfg0.get("dy", 0))
            scale = float(cfg0.get("scale", 1.0))
            old = text_elem.get("transform", "")
            tf = f" translate({dx},{dy}) scale({scale})"
...
if data_file:
    try:
        ...
        # show load message only to Editor (per request)
        if role == "Editor":
...
with left_col:
    ...
    if mapping_file is not None:
        try:
            ...
            if mapping_file.name.lower().endswith(".json"):
                ...
                # expect dict {placeholder: {col, align, dx, dy, scale}}
    ...
    # Download mapping buttons (JSON + CSV)
    st.write("")  # spacer
...
with right_col:
    ...
    if not sanitized_template:
        ...
        # find first row that has a mapped placeholder value
...
if st.button("Generate") and sanitized_template and df is not None and not df.empty and st.session_state.get("mapping"):
    ...
    for idx, row in df.iterrows():
        ...
        if not matched:
            continue  # don't generate for rows that don't have mapped placeholder values
        try:
    ...
    if export_mode == "Single combined PDF" and pdf_pages:
        combined = None
        try:
            # try PyPDF2 first (widely available)
            ...
            # fallback: return separate pages if combine fails
